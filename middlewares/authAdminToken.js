...
function authAdminToken(req, res, next) {
    ...
    try {
        ...
        // check if eid (employee id) exists and if isAdmin (column) of employee is equal to predifined admin id
        if (!decoded.eid || decoded.isAdmin !== constants.ADMIN_ID) 
            return res.status(401).send('Access denied. Not allowed to perform operation.');
